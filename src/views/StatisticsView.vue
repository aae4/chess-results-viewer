          ...
          <v-card>
            ...
            <v-card-text>
              ...
              <v-list lines="two" density="compact">
                <v-list-item class="list-item-hover" v-if="stats.longestGame" prepend-icon="mdi-timer-sand-complete" @click="goToGame(stats.longestGame.game.id)">
                ...
                <v-list-item class="list-item-hover" v-if="stats.shortestGame" prepend-icon="mdi-timer-sand-empty" @click="goToGame(stats.shortestGame.game.id)">
            ...
            <v-data-table v-if="display.mdAndUp.value" :headers="openingsHeaders" :items="stats.topOpenings" :items-per-page="10" density="compact">
              ...
              <template v-slot:item.count="{ item }" v-if="stats.resultDistribution">{{ item.count }} ({{ ((item.count / totalGames) * 100).toFixed(0) }}%)</template>
            ...
            <v-list v-else lines="three" class="py-0">
              <template v-for="(item, i) in stats.topOpenings" :key="item.eco">
                <v-list-item>
                  ...
                  <v-list-item-subtitle class="mt-1">{{ item.eco }} · {{ item.count }} {{ item.count > 1 ? 'партии' : 'партия' }} ({{ ((item.count / totalGames) * 100).toFixed(0) }}%)</v-list-item-subtitle>
...
const totalGames = computed(() => stats.value ? stats.value.resultDistribution.totalGames : 0);
